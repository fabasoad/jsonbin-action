...
describe('Test JSONbin client', () => {
  ...
  it('CRUD works as expected', async () => {
    ...
    const resp1 = await jsonbin.create(process.env.API_KEY, JSON.stringify(expected));
    ...
    const resp2 = await jsonbin.update(process.env.API_KEY, resp1.id, JSON.stringify(expected));
    const actual2 = await getBin(resp2.url);
    expect(expected).to.deep.equal(actual2);
  });

  after(async () => {
    await Promise.all(binToDelete.map(id => jsonbin.delete(process.env.API_KEY, id)));
  });
});